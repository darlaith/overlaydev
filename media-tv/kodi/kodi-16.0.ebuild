...
LICENSE="GPL-2"
SLOT="0"
IUSE="airplay +alsa avahi bluetooth bluray caps +cec css dbus debug gles java joystick midi mysql +nfs opengl profile -projectm pulseaudio rpi rpi2 rtmp +samba sftp test +texturepacker udisks upnp upower +usb vaapi vdpau webserver -X rpi"
REQUIRED_USE="
	|| ( gles opengl )
	|| ( rpi rpi2 )
	gles? ( !vaapi )
	...
	udisks? ( dbus )
	upower? ( dbus )
	rpi? ( gles alsa cec !vaapi !vdpau !X !joystick !bluray !css )
	rpi2? ( gles alsa cec !vaapi !vdpau !X !joystick !bluray !css )
"
...
COMMON_DEPEND="${PYTHON_DEPS}
	...
	rpi? ( media-video/omxplayer )
	rpi2? ( media-video/omxplayer )	
